...
func TestReceiveValidResponse(t *testing.T) {
	...
	if result != nil {
		t.Errorf("Result should have passed since we received a valid response")
	}
}

func TestFailedResponses(t *testing.T) {
	tables := []struct {
		failedResponse string
		expectedType   error
	}{
		{`{"result":null,"error":{"code":-32601,"message":"Method not found"},"id":null}`, ErrMethodRequestNotFound},
		{`{"result":null,"error":{"code":-3,"message":""},"id":null}`, ErrIncorrectInput},
		{`{"result":null,"error":{"code":-5,"message":""},"id":null}`, ErrInvalidAddr},
		{`{"result":null,"error":{"code":-8,"message":""},"id":null}`, ErrInvalidParameter},
		{`{"result":null,"error":{"code":502,"message":""},"id":null}`, ErrUnresponsive},
	}

	for _, table := range tables {
		expectedError := bitcoinClient.AuthenticateRPCResponse(table.failedResponse)

		if expectedError == nil {
			t.Errorf("Expected an Error, this should NOT be nil")
		}

		if expectedError != table.expectedType {
