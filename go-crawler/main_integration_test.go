...
func (suite *MainIntegrationSuite) TestDBConnection() {
	dbClient, err := controllers.NewDBClient(
		controllers.DBPort(injector.PostgresPort()),
		controllers.DBName(injector.PostgresDBName()),
		controllers.DBUser(injector.PostgresUserName()),
		controllers.DBHost(injector.PostgresDomain()),
		controllers.DBPassword(injector.PostgresPassword()),
		controllers.DBType("postgres"),
		controllers.PostgresClient())
...
// TestDBConnectionShouldFail will since we are trying to connect to a db that does not exist.
func (suite *MainIntegrationSuite) TestDBConnectionShouldFail() {
	dbClient, err := controllers.NewDBClient(
		...
		controllers.DBHost(injector.PostgresDomain()),
		controllers.DBPassword(injector.PostgresPassword()),
		controllers.DBType("db does not exist"),
		controllers.PostgresClient())
